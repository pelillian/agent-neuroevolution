    ...
    while True:
        ...
        for env_i, env in enumerate(envs):
            with sessions[env_i][curr_parent].as_default():
                ...
                print('env_id {}'.format(env.unwrapped.spec.id))
...
def run_worker(master_redis_cfg, relay_redis_cfg, noise, *, min_task_runtime=.2):
    ...
    while True:
        ...
        for i, env_i in enumerate(envs):
            if env_i.unwrapped.spec.id == task_data.env_id:
                env = env_i
                policy = policies[i]
                sess = sessions[i]

        assert policy is not None and env is not None and sess is not None
